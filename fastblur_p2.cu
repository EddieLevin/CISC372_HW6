...
int main(int argc,char** argv){
    ...
    cudaMallocManaged(&dev_img,sizeof(uint8_t)*pWidth*height);
    cudaMallocManaged(&dev_dest, sizeof(float)*pWidth*height);   
    cudaMallocManaged(&mid, sizeof(float)*pWidth*height);
    cudaMallocManaged(&dest, sizeof(float)*pWidth*height);
    cudaMemcpy(dev_img,img,pWidth*height*sizeof(uint8_t),cudaMemcpyHostToDevice);
    stbi_image_free(img); 
    dest=(float*)malloc(sizeof(float)*pWidth*height); 
    t1=time(NULL);
    int numBlocks = (pWidth+255)/256;
    computeColumn<<<numBlocks,256>>>(dev_img,mid,pWidth,height,radius,bpp);
    cudaDeviceSynchronize();

    numBlocks = (height+255)/256;
    cudaMemcpy(dest,mid,sizeof(float)*pWidth*height,cudaMemcpyDeviceToHost );
    computeRow<<<numBlocks,256>>>(mid,dev_dest,pWidth,height,radius,bpp);
    cudaDeviceSynchronize();
    cudaMemcpy(dest,dev_dest,sizeof(float)*pWidth*height,cudaMemcpyDeviceToHost );
