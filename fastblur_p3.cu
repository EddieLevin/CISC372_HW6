...
int main(int argc,char** argv){
    ...
    printf("%d",pWidth);
    cudaMalloc(&dev_img,sizeof(uint8_t)*pWidth*height);
    cudaMalloc(&dev_dest, sizeof(float)*pWidth*height);   
    ...
    computeColumn<<<numBlocks,blockSize>>>(dev_img,mid,pWidth,height,radius,bpp);
    cudaDeviceSynchronize();
    cudaMemcpy(dest,mid,sizeof(float)*pWidth*height,cudaMemcpyDeviceToHost);

    numBlocks = (height+blockSize-1)/blockSize;

    computeRow<<<numBlocks,blockSize>>>(mid,dev_dest,pWidth,height,radius,bpp);
    cudaDeviceSynchronize();
